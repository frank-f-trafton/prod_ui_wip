...
function plan.make(parent)
	...
	if content then
		...
		-- Repeat-Button
		local b_rep = content:addChild("base/button_repeat")
		b_rep.x = 0
		b_rep.y = 0
		b_rep.w = 128
		b_rep.h = 64
		...
		local chk_m = content:addChild("base/checkbox_multi")
		chk_m.x = 160
		chk_m.y = 96
		chk_m.w = 256
		chk_m.h = 64
		...
		py=py+py_plus
		rdo = content:addChild("base/radio_button")
		rdo.x = px
		rdo.y = py
		rdo.w = 192
		rdo.h = py_plus
		...
		-- Sticky Button.
		local sticky = content:addChild("base/button_sticky")
		sticky.x = 256
		sticky.y = 192
		sticky.w = 240
		sticky.h = 32
		...
		local b_unst = content:addChild("base/button")
		...
		local b_instant = content:addChild("base/button_instant")
		b_instant.x = 256
		...
		makeLabel(content, 256, 192+48+48+48, 240, 64, "Right-click, middle-click, or hit the 'application' key while the button is focused.", "multi")
		local b_secondary = content:addChild("base/button")
		b_secondary.x = 256
		...
		local btn_2c = content:addChild("base/button_double_click")
		btn_2c.x = 256
