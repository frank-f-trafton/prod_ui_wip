-- More keyboard state for the UI Context.
...
local key_constants = require(context.conf.prod_ui_req .. "data.keyboard.scancodes")
local scancodes = require(context.conf.prod_ui_req .. "data.keyboard.scancodes")
...
function _mt_mgr:keyDown(owner, kc, sc, isrepeat)
	...
	if not mods[check_mod_key] then
		hot_kc = uiKeyboard.getKeyString(mods["ctrl"], mods["shift"], mods["alt"], mods["gui"], false, kc)
		hot_sc = uiKeyboard.getKeyString(mods["ctrl"], mods["shift"], mods["alt"], mods["gui"], true, sc)
	end
