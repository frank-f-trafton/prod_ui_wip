-- WIP: This widget doesn't work yet.

--[[
A rolling message log.

┌──────────────────────────────────┬─┐
│ [I] Quick brown fox seen jumping │^│
│      over lazy dog               ├─┤
│ [I] I like pizza                 │ │
│:[I]:Unscrupulous:conduct:::::::::│ │
│                                  ├─┤
│                                  │v│
├─┬──────────────────────────────┬─┼─┤
...
local function _shapeItem(self, item)
	...
	local width, wrapped = font:getWrap(item.text, self.vp.w) -- TODO: check arg #2

	item.w = width
	item.h = math.floor((font:getHeight() * font:getLineHeight() * #wrapped) + skin.item_pad_v)
end


local function _cullEntries(items, max)
	local n = #items
	if n > max then
		local i = n - max
		while i > 0 do
...
function def:replaceLastItem(text, icon_id)
	uiAssert.loveStringOrColoredText(1, text)
	uiAssert.typeEval(2, icon_id, "string")
...
function def:evt_initialize()
	...
	wcMenu.setup(self, nil, true) -- with mark state ("cursor" mode only)
...
function def:evt_keyPressed(targ, key, scancode, isrepeat)
	if self == targ then
		...
		if self:cb_keyPressed(key, scancode, isrepeat)
...
def.default_skinner = {
	validate = uiSchema.newKeysX {
		...
		default_icon_side = {uiAssert.oneOfEval, "left", "right"},
		default_text_align = {uiAssert.oneOfEval, "left", "center", "right"},
